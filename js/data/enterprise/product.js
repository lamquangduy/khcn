...
const tableName = 'khcn_data_enterprise_product'; // Tên bảng trong Supabase
let currentPage = 1;
...
async function loadProducts() {
    try {
       ...
       const response = await fetch(`https://data.vunhat2009.workers.dev/${tableName}`);
        ...
        alert('Không thể tải dữ liệu sản phẩm/dịch vụ. Vui lòng thử lại sau.');
    } finally {
        hideLoading();
    }
}
        ...
        filteredProducts = allProducts.filter(product => {
            ...
            if (filters.search) {
                const searchMatch =
                    ...
                    product.enterprise.toLowerCase().includes(filters.search);
   ...
   const product = allProducts.find(i => i.id === id);
    ...
    const modalContent = `
        <div class="modal-header">
             <h5 class="modal-title">Chi tiết sản phẩm dịch vụ</h5>
         ...
         <div class="modal-body">
            ...
            <div class="mb-3">
                 <label class="fw-bold">Tên sản phẩm/dịch vụ:</label>
                 <p>${product.name}</p>
            </div>
             <div class="mb-3">
                  <label class="fw-bold">Doanh nghiệp:</label>
                  <p>${product.enterprise}</p>
              </div>
              <div class="mb-3">
...
async function addNewProduct() {
     const modalContent = `
         <div class="modal-header">
            <h5 class="modal-title">Thêm mới sản phẩm dịch vụ</h5>
             <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
            <form id="addProductForm" class="needs-validation" novalidate>
                 <div class="mb-3">
                      <label class="form-label required">Tên sản phẩm/dịch vụ</label>
                    <input type="text" class="form-control" name="name" required>
                    <div class="invalid-feedback">Vui lòng nhập tên sản phẩm/dịch vụ</div>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label class="form-label required">Doanh nghiệp</label>
                     <input type="text" class="form-control" name="enterprise" required>
                     <div class="invalid-feedback">Vui lòng nhập doanh nghiệp</div>
                </div>
                 <div class="mb-3">
...
async function saveNewProduct() {
     try {
        ...
        showLoading();
        const formData = {
            id: `SP${String(allProducts.length + 1).padStart(3, '0')}`,
        ...
        alert('Thêm mới sản phẩm/dịch vụ thành công!');

    } catch (error) {
         console.error('Lỗi khi thêm mới sản phẩm/dịch vụ:', error);
        alert('Có lỗi xảy ra khi thêm mới sản phẩm/dịch vụ');
    } finally {
        hideLoading();
    }
}
        ...
        <div class="modal-header">
         ...
         <div class="modal-body">
            <form id="editProductForm" class="needs-validation" novalidate>
                ...
                <div class="mb-3">
                     <label class="form-label required">Doanh nghiệp</label>
                     <input type="text" class="form-control" name="enterprise" value="${product.enterprise}" required>
                    <div class="invalid-feedback">Vui lòng nhập doanh nghiệp</div>
                </div>
               <div class="mb-3">
                     <label class="form-label required">Loại hình</label>
...
async function saveEditedProduct(id) {
    try {
        ...
        alert('Cập nhật thông tin sản phẩm/dịch vụ thành công!');

    } catch (error) {
         console.error('Lỗi khi cập nhật thông tin sản phẩm/dịch vụ:', error);
        alert('Có lỗi xảy ra khi cập nhật thông tin sản phẩm/dịch vụ');
    } finally {
        hideLoading();
    }
}
...
async function deleteProduct(id) {
     try {
          const product = allProducts.find(i => i.id === id);
          if (!product) {
            alert('Không tìm thấy thông tin sản phẩm/dịch vụ');
             return;
         }

        if (!confirm(`Bạn có chắc chắn muốn xóa sản phẩm/dịch vụ này?\n\n- Mã: ${product.id}\n- Tên sản phẩm/dịch vụ: ${product.name}`)) {
            return;
        }
        showLoading();
          const response = await fetch(`https://data.vunhat2009.workers.dev/${tableName}`, {
       ...
       alert('Xóa sản phẩm/dịch vụ thành công!');

    } catch (error) {
         console.error('Lỗi khi xóa sản phẩm/dịch vụ:', error);
        alert('Có lỗi xảy ra khi xóa sản phẩm/dịch vụ');
    } finally {
        hideLoading();
    }
}
...
function updatePagination() {
    ...
    paginationInfo.textContent = `Hiển thị ${startItem}-${endItem} trong tổng số ${filteredProducts.length} sản phẩm/dịch vụ`;
