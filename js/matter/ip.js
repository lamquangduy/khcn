let allCases = [];
let filteredCases = [];
const tableName = 'khcn_matter_ip';
let currentPage = 1;
...
async function loadCases() {
    try {
        ...
        // Sắp xếp theo id
        allCases.sort((a, b) => {
            const idA = parseInt(a.id.slice(4), 10);
            const idB = parseInt(b.id.slice(4), 10);
            return idA - idB;
        });
        ...
        alert('Không thể tải dữ liệu sự vụ sở hữu trí tuệ. Vui lòng thử lại sau.');
    } finally {
        hideLoading();
    }
}
...
function addNewCase() {
    ...
    const modalContent = `
         <div class="modal-header">
            <h5 class="modal-title">Thêm mới sự vụ sở hữu trí tuệ</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
                 ...
                 <div class="row mb-3">
                     <div class="col-md-6">
                        ...
                        <select class="form-select" name="department" required>
                            ...
                            <option value="Phòng Quản lý SHTT">Phòng Quản lý SHTT</option>
           ...
           <form id="editCaseForm" class="needs-validation" novalidate>
                ...
                <div class="row mb-3">
                   <div class="col-md-6">
                        <label class="form-label required">Loại sự vụ</label>
                        <select class="form-select" name="type" required>
                            <option value="patent" ${caseData.type === 'patent' ? 'selected' : ''}>Bằng sáng chế</option>
                            <option value="solution" ${caseData.type === 'solution' ? 'selected' : ''}>Giải pháp hữu ích</option>
                             <option value="trademark" ${caseData.type === 'trademark' ? 'selected' : ''}>Nhãn hiệu</option>
                ...
                <div class="row mb-3">
                 ...
                 <div class="row mb-3">
                      <div class="col-md-6">
                         <label class="form-label required">Đơn vị xử lý</label>
                          <select class="form-select" name="department" required>
                              <option value="Phòng Quản lý SHTT" ${caseData.department === 'Phòng Quản lý SHTT' ? 'selected' : ''}>Phòng Quản lý SHTT</option>
...
function generateNextId() {
    ...
    return `SHTT${String(nextNumericPart).padStart(3, '0')}`;
}
