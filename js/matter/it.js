let allCases = [];
let filteredCases = [];
const tableName = 'khcn_matter_it';
let currentPage = 1;
        ...
        filteredCases = allCases.filter(case_ => {
            ...
            if (filters.date) {
             ...
             if (filters.department) {
                const deptMap = {
                    'dept1': 'Phòng CNTT',
                    'dept2': 'Trung tâm CNTT',
                    'dept3': 'Bộ phận hỗ trợ kỹ thuật',
                };
...
function addNewCase() {
    ...
    const modalContent = `
        <div class="modal-header">
            <h5 class="modal-title">Thêm mới sự vụ CNTT</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
            <form id="addCaseForm" class="needs-validation" novalidate>
                ...
                <div class="row mb-3">
                     <div class="col-md-6">
                        ...
                        <select class="form-select" id="caseType" name="type" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="department" required>
                            <option value="">Chọn đơn vị xử lý</option>
                             ...
                             <option value="Bộ phận hỗ trợ kỹ thuật">Bộ phận hỗ trợ kỹ thuật</option>
                        </select>
                     ...
                     <div class="col-md-6">
                        <label class="form-label required">Loại sự vụ</label>
                         <select class="form-select" name="type" required>
                            <option value="software" ${caseData.case_type === 'software' ? 'selected' : ''}>Phần mềm</option>
                            <option value="hardware" ${caseData.case_type === 'hardware' ? 'selected' : ''}>Phần cứng</option>
                            <option value="network" ${caseData.case_type === 'network' ? 'selected' : ''}>Mạng</option>
                            <option value="security" ${caseData.case_type === 'security' ? 'selected' : ''}>An ninh</option>
                ...
                <!-- Thông tin xử lý -->
                 <div class="row mb-3">
                     <div class="col-md-6">
                         <label class="form-label required">Đơn vị xử lý</label>
                        <select class="form-select" name="department" required>
                            <option value="Phòng CNTT" ${caseData.department === 'Phòng CNTT' ? 'selected' : ''}>Phòng CNTT</option>
                             <option value="Trung tâm CNTT" ${caseData.department === 'Trung tâm CNTT' ? 'selected' : ''}>Trung tâm CNTT</option>
                             <option value="Bộ phận hỗ trợ kỹ thuật" ${caseData.department === 'Bộ phận hỗ trợ kỹ thuật' ? 'selected' : ''}>Bộ phận hỗ trợ kỹ thuật</option>
                        </select>
                        <div class="invalid-feedback">Vui lòng chọn đơn vị xử lý</div>
                    </div>
                     <div class="col-md-6">
...
async function deleteCase(id) {
    try {
        ...
        if (!confirm(`Bạn có chắc chắn muốn xóa sự vụ sau?\n\n` +
            ...
            `- Loại sự vụ: ${formatCaseType(caseData.case_type)}\n` +
...
function generateNextId() {
    ...
    return `CNTT${String(nextNumericPart).padStart(3, '0')}`;
}