let allCases = [];
let filteredCases = [];
const tableName = 'khcn_matter_research';
let currentPage = 1;
...
function displayCases() {
    ...
    displayedData.forEach((case_, index) => {
        ...
        row.innerHTML = `
            ...
            <td>${case_.title}</td>
            <td>${formatCaseType(case_.type)}</td>
             <td>${formatDate(case_.discoverydate)}</td>
...
async function filterCases() {
      try {
        ...
        filteredCases = allCases.filter(case_ => {
            ...
            if (filters.type && case_.type !== filters.type) matches = false;
            ...
            if (filters.source && case_.source !== filters.source) matches = false;
             ...
             if (filters.date) {
                 ...
                 const caseDate = new Date(case_.discoverydate).setHours(0,0,0,0);
                if (filterDate !== caseDate) matches = false;
            }

            if (filters.department) {
                const deptMap = {
                   'dept1': 'Phòng Quản lý KHCN',
        ...
        <div class="modal-body">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <p>${formatCaseType(caseData.type)}</p>
                </div>
                <div class="col-md-6">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <p>${formatDate(caseData.discoverydate)}</p>
                </div>
                <div class="col-md-6">
...
function addNewCase() {
    ...
    const modalContent = `
        <div class="modal-header">
            <h5 class="modal-title">Thêm mới sự vụ nghiên cứu khoa học</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
                   ...
                   <div class="col-md-6">
                        ...
                        <select class="form-select" name="type" required>
                           <option value="">Chọn loại sự vụ</option>
                            ...
                            <option value="work">Công trình nghiên cứu</option>
                            <option value="topic">Đề tài nghiên cứu</option>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="department" required>
                            ...
                            <option value="Phòng Quản lý KHCN">Phòng Quản lý KHCN</option>
        ...
        showLoading();
         const formData = {
           ...
           title: form.querySelector('[name="title"]').value,
            ...
            discoverydate: form.querySelector('[name="discoverydate"]').value,
...
function editCase(id) {
    ...
    // Tạo modal form với dữ liệu hiện tại
    const modalContent = `
        <div class="modal-header">
            ...
            <form id="editCaseForm" class="needs-validation" novalidate>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label class="form-label required">Loại sự vụ</label>
                         <select class="form-select" name="type" required>
                            <option value="project" ${caseData.type === 'project' ? 'selected' : ''}>Dự án nghiên cứu</option>
                            <option value="work" ${caseData.type === 'work' ? 'selected' : ''}>Công trình nghiên cứu</option>
                            <option value="topic" ${caseData.type === 'topic' ? 'selected' : ''}>Đề tài nghiên cứu</option>
                            <option value="center" ${caseData.type === 'center' ? 'selected' : ''}>Trung tâm nghiên cứu</option>
                            <option value="lab" ${caseData.type === 'lab' ? 'selected' : ''}>Phòng thí nghiệm</option>
                             <option value="other" ${caseData.type === 'other' ? 'selected' : ''}>Khác</option>
                        </select>
                        <div class="invalid-feedback">Vui lòng chọn loại sự vụ</div>
                    </div>
                     <div class="col-md-6">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" name="discoverydate" 
                               value="${caseData.discoverydate}" required
                ...
                <div class="row mb-3">
                   <div class="col-md-6">
                        <label class="form-label required">Đơn vị xử lý</label>
                         <select class="form-select" name="department" required>
                             <option value="Phòng Quản lý KHCN" ${caseData.department === 'Phòng Quản lý KHCN' ? 'selected' : ''}>Phòng Quản lý KHCN</option>
...
function formatCaseType(type) {
    const types = {
        ...
        work: 'Công trình nghiên cứu',
        topic: 'Đề tài nghiên cứu',
        center: 'Trung tâm nghiên cứu',
...
function generateNextId() {
    ...
    return `NC${String(nextNumericPart).padStart(3, '0')}`;
}