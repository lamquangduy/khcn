let allCases = [];
let filteredCases = [];
const tableName = 'khcn_matter_telecom';
let currentPage = 1;
...
async function filterCases() {
    try {
        ...
        filteredCases = allCases.filter(case_ => {
            ...
            if (filters.provider && case_.provider !== filters.provider) matches = false;
...
function addNewCase() {
    ...
    const modalContent = `
        <div class="modal-header">
            <h5 class="modal-title">Thêm mới sự vụ viễn thông</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
            ...
            <form id="editCaseForm" class="needs-validation" novalidate>
                ...
                <div class="row mb-3">
                   <div class="col-md-6">
                         <label class="form-label required">Loại sự vụ</label>
                        <select class="form-select" name="type" required>
                            <option value="infrastructure" ${caseData.case_type === 'infrastructure' ? 'selected' : ''}>Hạ tầng</option>
                            <option value="service" ${caseData.case_type === 'service' ? 'selected' : ''}>Dịch vụ</option>
                            <option value="quality" ${caseData.case_type === 'quality' ? 'selected' : ''}>Chất lượng</option>
                ...
                <!-- Thông tin xử lý -->
                 <div class="row mb-3">
                    <div class="col-md-6">
                        <label class="form-label required">Nhà cung cấp</label>
                        <select class="form-select" name="provider" required>
                           <option value="Viettel" ${caseData.provider === 'Viettel' ? 'selected' : ''}>Viettel</option>
                           <option value="VNPT" ${caseData.provider === 'VNPT' ? 'selected' : ''}>VNPT</option>
                            <option value="Mobifone" ${caseData.provider === 'Mobifone' ? 'selected' : ''}>Mobifone</option>
                             <option value="FPT" ${caseData.provider === 'FPT' ? 'selected' : ''}>FPT</option>
                        </select>
                        <div class="invalid-feedback">Vui lòng chọn nhà cung cấp</div>
                    </div>
                      <div class="col-md-6">
...
function generateNextId() {
    ...
    return `VT${String(nextNumericPart).padStart(3, '0')}`;
}