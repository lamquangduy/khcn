let allCases = [];
let filteredCases = [];
const tableName = 'khcn_matter_postal';
let currentPage = 1;
...
function initializeEventListeners() {
    ...
    // REMOVED event listener for 'provider' filter as it's removed from HTML
...
async function filterCases() {
    try {
        ...
        const filters = {
            ...
            department: document.getElementById('department').value, // Changed from provider to department
        ...
        filteredCases = allCases.filter(case_ => {
            ...
            if (filters.department && case_.department !== filters.department) matches = false; // Changed from provider to department
    ...
    const modalContent = `
        ...
        <div class="modal-body">
            ...
            <div class="mb-3">
             ...
             <!-- Nhà cung cấp field removed from view modal -->
        </div>
...
function addNewCase() {
    ...
    const modalContent = `
        <div class="modal-header">
            <h5 class="modal-title">Thêm mới sự vụ bưu chính</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body">
                ...
                <div class="row mb-3">
                     <div class="col-md-6">
                        ...
                        <select class="form-select" name="type" required>
                            ...
                            <option value="office">Bưu cục, điểm phục vụ</option>
               ...
               <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select class="form-select" name="department" required>
                            ...
                            <option value="Phòng Quản lý Bưu chính">Phòng Quản lý Bưu chính</option>
                            <option value="Thanh tra Sở TT&TT">Thanh tra Sở TT&TT</option>
                            <option value="Trung tâm CNTT">Trung tâm CNTT</option>
                        </select>
                ...
                <div class="mb-3">
                 ...
                 <!-- Nhà cung cấp field removed from add new modal -->
            </form>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
...
async function saveNewCase() {
    try {
        ...
        const formData = {
            ...
            department:  document.getElementById('department').value, // Get department from UI
            source: form.querySelector('[name="source"]').value,
            location: form.querySelector('[name="location"]').value,
            description: form.querySelector('[name="description"]').value,
            // provider: form.querySelector('[name="provider"]').value  <--- Removed from formData
        };
...
function editCase(id) {
            ...
            <form id="editCaseForm" class="needs-validation" novalidate>
                ...
                <div class="row mb-3">
                   <div class="col-md-6">
                         <label class="form-label required">Loại sự vụ</label>
                        <select class="form-select" name="type" required>
                            <option value="enterprise" ${caseData.case_type === 'enterprise' ? 'selected' : ''}>Doanh nghiệp</option>
                            <option value="office" ${caseData.case_type === 'office' ? 'selected' : ''}>Bưu cục, điểm phục vụ</option>
                            <option value="service" ${caseData.case_type === 'service' ? 'selected' : ''}>Dịch vụ</option>
                ...
                <!-- Thông tin xử lý -->
                 <div class="row mb-3">
                    <div class="col-md-6">
                         <label class="form-label required">Đơn vị xử lý</label>
                        <select class="form-select" name="department" required>
                            <option value="Phòng Quản lý Bưu chính" ${caseData.department === 'Phòng Quản lý Bưu chính' ? 'selected' : ''}>Phòng Quản lý Bưu chính</option>
                            <option value="Thanh tra Sở TT&TT" ${caseData.department === 'Thanh tra Sở TT&TT' ? 'selected' : ''}>Thanh tra Sở TT&TT</option>
                            <option value="Trung tâm CNTT" ${caseData.department === 'Trung tâm CNTT' ? 'selected' : ''}>Trung tâm CNTT</option>
                        </select>
                        <div class="invalid-feedback">Vui lòng chọn đơn vị xử lý</div>
                    </div>
                      <div class="col-md-6">
                ...
                <div class="mb-3">
                 ...
                 <!-- Nhà cung cấp field removed from edit modal -->
            </form>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button>
    ...
    // Khởi tạo tooltips và validation
    const modal = new bootstrap.Modal(modalElement);
    modal.show();
...
function formatCaseType(type) {
    const types = {
        enterprise: 'Doanh nghiệp bưu chính',
        office: 'Bưu cục, điểm phục vụ',
        service: 'Dịch vụ bưu chính',
        other: 'Khác'
    };
